...
function dateMatchModifiers(date, matchers, dateLib$1 = defaultDateLib) {
	...
	return matchersArr.some((matcher) => {
		...
		if (isDatesArray(matcher, dateLib$1)) return matcher.includes(date);
...
function createGetModifiers(days, props, navStart, navEnd, dateLib$1) {
	...
	for (const day of days) {
		...
		const isToday = isSameDay$1(date, today ?? dateLib$1.today());
...
*/
function getMonthOptions(displayMonth, navStart, navEnd, formatters, dateLib$1) {
	...
	return eachMonthOfInterval$1({
		...
		const label = formatters.formatMonthDropdown(month, dateLib$1);
		const value = getMonth$1(month);
		const disabled = navStart && month < startOfMonth$1(navStart) || navEnd && month > startOfMonth$1(navEnd) || false;
		return {
			value,
			label,
			disabled
...
*/
function getWeekdays(dateLib$1, ISOWeek, broadcastCalendar) {
	const today = dateLib$1.today();
	const start = broadcastCalendar ? dateLib$1.startOfBroadcastWeek(today, dateLib$1) : ISOWeek ? dateLib$1.startOfISOWeek(today) : dateLib$1.startOfWeek(today);
	const days = [];
	for (let i = 0; i < 7; i++) {
...
*/
function getYearOptions(navStart, navEnd, formatters, dateLib$1, reverse = false) {
	...
	const firstNavYear = startOfYear$1(navStart);
	const lastNavYear = endOfYear$1(navEnd);
	...
	return years.map((year) => {
		const label = formatters.formatYearDropdown(year, dateLib$1);
		return {
...
function getDates(displayMonths, maxDate, props, dateLib$1) {
	...
	const endWeekLastDate = broadcastCalendar ? endOfBroadcastWeek$1(lastMonth) : ISOWeek ? endOfISOWeek$1(endOfMonth$1(lastMonth)) : endOfWeek$1(endOfMonth$1(lastMonth));
	const nOfDays = differenceInCalendarDays$1(endWeekLastDate, startWeekFirstDate);
...
function getInitialMonth(props, navStart, navEnd, dateLib$1) {
	...
	if (navEnd && differenceInCalendarMonths$1(navEnd, initialMonth) < numberOfMonths - 1) {
		const offset = -1 * (numberOfMonths - 1);
		initialMonth = addMonths$1(navEnd, offset);
	}
...
function getMonths(displayMonths, dates, props, dateLib$1) {
	...
	const dayPickerMonths = displayMonths.reduce((months, month) => {
		...
		const weeks = monthDates.reduce((weeks$1, date) => {
			const weekNumber = props.ISOWeek ? getISOWeek$1(date) : getWeek$1(date);
			const week = weeks$1.find((week$1) => week$1.weekNumber === weekNumber);
			const day = new CalendarDay(date, month, dateLib$1);
			if (!week) weeks$1.push(new CalendarWeek(weekNumber, [day]));
...
function useCalendar(props, dateLib$1) {
	...
	(0, import_react.useEffect)(() => {
		const newInitialMonth = getInitialMonth(props, navStart, navEnd, dateLib$1);
		setFirstMonth(newInitialMonth);
	}, [props.timeZone]);
	/** The months displayed in the calendar. */
	const displayMonths = getDisplayMonths(firstMonth, navEnd, props, dateLib$1);
	/** The dates displayed in the calendar. */
	const dates = getDates(displayMonths, props.endMonth ? endOfMonth$1(props.endMonth) : void 0, props, dateLib$1);
	/** The Months displayed in the calendar. */
	const months = getMonths(displayMonths, dates, props, dateLib$1);
	/** The Weeks displayed in the calendar. */
	const weeks = getWeeks(months);
	/** The Days displayed in the calendar. */
	const days = getDays(months);
	const previousMonth = getPreviousMonth(firstMonth, navStart, props, dateLib$1);
	const nextMonth = getNextMonth(firstMonth, navEnd, props, dateLib$1);
...
function useRange(props, dateLib$1) {
	...
	const select = (triggerDate, modifiers, e) => {
		const { min: min$1, max: max$1 } = props;
		const newRange = triggerDate ? addToRange(triggerDate, selected, min$1, max$1, required, dateLib$1) : void 0;
...
function DayPicker(initialProps) {
	...
	if (props.timeZone) {
		props = { ...initialProps };
		if (props.today) props.today = new TZDate(props.today, props.timeZone);
		if (props.month) props.month = new TZDate(props.month, props.timeZone);
		if (props.defaultMonth) props.defaultMonth = new TZDate(props.defaultMonth, props.timeZone);
		if (props.startMonth) props.startMonth = new TZDate(props.startMonth, props.timeZone);
		if (props.endMonth) props.endMonth = new TZDate(props.endMonth, props.timeZone);
		if (props.mode === "single" && props.selected) props.selected = new TZDate(props.selected, props.timeZone);
		else if (props.mode === "multiple" && props.selected) props.selected = props.selected?.map((date) => new TZDate(date, props.timeZone));
	...
	const { components, formatters, labels, dateLib: dateLib$1, locale, classNames } = (0, import_react.useMemo)(() => {
		const locale$1 = {
			...enUS,
			...props.locale
		};
	...
	const weekdays = (0, import_react.useMemo)(() => getWeekdays(dateLib$1, props.ISOWeek), [dateLib$1, props.ISOWeek]);
	...
	const handleMonthChange = (0, import_react.useCallback)((date) => (e) => {
		...
		const month = dateLib$1.setMonth(dateLib$1.startOfMonth(date), selectedMonth);
		goToMonth(month);
	}, [dateLib$1, goToMonth]);
	const handleYearChange = (0, import_react.useCallback)((date) => (e) => {
		const selectedYear = Number(e.target.value);
		const month = dateLib$1.setYear(dateLib$1.startOfMonth(date), selectedYear);
		goToMonth(month);
	}, [dateLib$1, goToMonth]);
	...
	return import_react.createElement(dayPickerContext.Provider, { value: contextValue }, import_react.createElement(components.Root, {
		...
		return import_react.createElement(components.Month, {
			...
			const monthControl = captionLayout === "dropdown" || captionLayout === "dropdown-months" ? import_react.createElement(components.MonthsDropdown, {
				...
				options: getMonthOptions(calendarMonth.date, navStart, navEnd, formatters, dateLib$1),
			...
			const yearControl = captionLayout === "dropdown" || captionLayout === "dropdown-years" ? import_react.createElement(components.YearsDropdown, {
				...
				options: getYearOptions(navStart, navEnd, formatters, dateLib$1, Boolean(props.reverseYears)),
				style: styles?.[UI.Dropdown],
			...
			return import_react.createElement(components.Week, {
				...
				return import_react.createElement(components.Day, {
					key: `${dateLib$1.format(date, "yyyy-MM-dd")}_${dateLib$1.format(day.displayMonth, "yyyy-MM")}`,
					day,
					modifiers,
					...
					"data-day": dateLib$1.format(date, "yyyy-MM-dd"),
					"data-month": day.outside ? dateLib$1.format(date, "yyyy-MM") : void 0,
